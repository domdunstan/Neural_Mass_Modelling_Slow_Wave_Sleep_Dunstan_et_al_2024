function varargout=sigstar2(groups,stats,xxx,nosort)
...
%     yd=myRange(y)*0.05; %separate sig bars vertically by 5% 
    yd=myRange(y)*0.02; %separate sig bars vertically by 5% 

    for ii=1:length(groups)
        thisY=findMinY(xlocs(ii,:))+yd;
        H(ii,:)=makeSignificanceBar(xlocs(ii,:),thisY+xxx,stats(ii));
    end
    %-----------------------------------------------------
